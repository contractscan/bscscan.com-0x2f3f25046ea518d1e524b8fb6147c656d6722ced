...
contract ERC20Mintable is ERC20, MinterRole,ReentrancyGuard {
    ...
    uint256 private _transactFeeValue = 5;
    uint private sale_date=0;
    ...
    function saleDate() public view returns (uint) {
        return sale_date;
    }
    ...
    function transactionFee(address from,address to,uint256 amount)internal returns (uint256) {
        ...
        if(setType==0&&sale_date>0){
            ...
            if(diff<=day.mul(3)){
                ...
                require(_limit[to]<=time.mul(BASE_RATIO), "_transfer:Limit exceeded");
            }
        }
...
contract MetaverseToken is ERC20, ERC20Detailed,ERC20Mintable {
    constructor(address dis) public ERC20Detailed("MT", "MT", 18) ERC20Mintable(dis) {
        uint256 totalSupply =22400* (10**uint256(18));
        _mint(address(this), totalSupply);
        _mint(dis, 8939* (10**uint256(18)));
        addMinter(dis);
    }
}